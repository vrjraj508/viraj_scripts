# =============================================================================
# SPRING BOOT GRADLE PROJECT GENERATOR
# Usage: .\spring-create-gradle.ps1 [ProjectName]
# =============================================================================
...
Write-Host "`n🚀 Creating Spring Boot GRADLE project: $ProjectName" -ForegroundColor Cyan
Write-Host "📦 Build Tool: Gradle (Kotlin DSL)" -ForegroundColor Green
Write-Host "📦 Artifact ID: $artifactId" -ForegroundColor Gray
Write-Host "📁 Package: $packageName" -ForegroundColor Gray
Write-Host ""

# Spring Initializr URL for GRADLE
...
# Download project
Write-Host "⬇️  Downloading from Spring Initializr..." -ForegroundColor Yellow
try {
    Invoke-WebRequest -Uri $url -OutFile "$ProjectName.zip" -ErrorAction Stop
} catch {
    Write-Host "❌ Failed to download project: $_" -ForegroundColor Red
    exit 1
}
...
Write-Host "⚙️  Configuring application.properties..." -ForegroundColor Yellow
...
if (-not $intellijFound) {
    Write-Host "   ⚠️  IntelliJ IDEA not found automatically" -ForegroundColor Yellow
...
Write-Host ""
Write-Host "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━" -ForegroundColor Gray
Write-Host "🎉 Happy coding with Gradle!" -ForegroundColor Green
```
...
| Feature | Maven Script | Gradle Script |
|---------|-------------|---------------|
| **Build file** | `pom.xml` | `build.gradle.kts` |
| **Wrapper** | `mvnw.cmd` | `gradlew.bat` |
| **Run command** | `.\mvnw.cmd spring-boot:run` | `.\gradlew.bat bootRun` |
| **Build speed** | Slower | ⚡ Faster |
| **Industry use** | More common | Growing |
...
```
C:\dev-tools\
├── spring-create-maven.ps1
└── spring-create-gradle.ps1